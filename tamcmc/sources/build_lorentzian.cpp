...
/*
VectorXd Lorentzian(const VectorXd x_l, const int l, const int m, const double H_l, const double gamma_l, const double fc_l, 
    const double a1, const double a2, const double a3, const double a4, const double a5, const double a6, 
    const double eta0, const double asym, const VectorXd& V){
    ...
    if(asym == 0){ //Model with no asymetry
        result= H_l*V(m+l)* ((tmp.setConstant(1) + profile)).cwiseInverse();
    } else{
        tmp.setConstant(1);
        asymetry=(tmp + asym*(x_l/fc_l - tmp)).array().square() + (tmp2.setConstant(0.5*gamma_l*asym/fc_l)).array().square();
        result=H_l*V(m+l)*asymetry.cwiseProduct(((tmp.setConstant(1) + profile)).cwiseInverse());
    }
    return result;
}
*/
...
// This parallelisation does not bring so much gain. In fact, there is some losss due to additional
// memory shufling between In/Out of Lorentzian()
/*
VectorXd build_l_mode_aj(const VectorXd& x_l, const double H_l, const double fc_l, 
        const double a1, const double a2, const double a3, const double a4, const double a5, const double a6, 
        const double eta0, const double asym, const double gamma_l, const int l, const VectorXd& V){
    ...
    for(int m=-l; m<=l; m++){
        VectorXd r=Lorentzian(x_l, l, m, H_l, gamma_l, fc_l, a1, a2, a3, a4, a5, a6, eta0, asym, V);
        #pragma omp critical
        result = result + r;
    }
return result;
...
*/
Optim_L optimum_lorentzian_calc_ajAlm(const VectorXd& x, const double H_l, const double fc_l, const double a1, 
    const double a3, const double a5, const double eta0, const double epsilon_nl, const VectorXd& thetas, const double asym, 
    const double gamma_l, const int l, const VectorXd& V, const double step, const double c, const std::string filter_type, gsl_funcs interp_funcs){
/*
    function that calculates the lorentzian on a optimized range of frequency. It returns a Vector of same size as the original vector x
