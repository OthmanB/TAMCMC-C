...
int main(int argc, char* argv[]){
	...
	//bool execute; // if argv[1]="execute" then look for argv[2]==execute. 
    ...
    desc.add_options()
		...
		("help,H", "Show help")
        ("execute,E", po::value<bool>()->implicit_value(1)->default_value(1), "Execute flag. If set to 0, the program stops after loading the configuration.")
	...
	if (vm.count("start_index") > 0) {
		startV = vm["start_index"].as<int>() - 1;
		if (startV < 0){
			std::cerr << "Error: The start_index number must be greater or equal than 1" << std::endl;
			std::cout << desc << std::endl;
			exit(EXIT_FAILURE); 
		}
	}
	if (vm.count("last_index") > 0) {
		lastV = vm["last_index"].as<int>() - 1;
		if (lastV < 0){
			std::cerr << "Error: The last_index number must be greater or equal than 1" << std::endl;
			std::cout << desc << std::endl;
			exit(EXIT_FAILURE); 
		}
	} 
	if (vm.count("startslice") > 0) {
		startslice = vm["startslice"].as<int>() - 1;
	}
	if (vm.count("lastslice") > 0) {
		lastslice = vm["lastslice"].as<int>() - 1;
	}
...
*/
	...
	if(vm["execute"].as<bool>() == false){
		std::cout << "Detection of execute=False... The code will exit now"<< std::endl;
		exit(EXIT_SUCCESS);
	}
