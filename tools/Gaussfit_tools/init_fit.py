...
def mode_initial_setup(spec_file, KIC_number, numax_guess, numax_uncertainty_guess, Amax_guess, outdir, 
					   fmin=0, fmax=5000, rebin=1, do_S1=None, datatype='sav',
					   do_model=True, do_data=True, extend_to_0=True):
	'''
	...
	if datatype == 'sav':
		...
		spec_reg=d['spec_reg']
	if datatype == 'data':
		freq, spec_reg, models, extended, true_xmin=read_data_file(spec_file, extend_to_0=extend_to_0)
	...
	if err == False:
		...
		if extended == False: 
			...
			if do_model == True:
				hdr=      "# File auto-generated by init_fit.py\n"
				hdr=hdr + "# Fit of Gaussian mode Envelope\n"
				hdr=hdr + "# ID:"+ str(KIC_number)+"\n# rebin="+str(rebin)+"\n"
				hdr=hdr + "# model_fullname= model_Harvey_Gaussian\n" 
				err=do_model_file(init_param, relax_param, name_param, prior_param, prior_name, outdir + "/" + KIC_number + "_Gaussfit.model", 
   				      np.min(freq[pos]), np.max(freq[pos]), header=hdr)
		# **************
	else:
